...
md`## Race bar com as vitórias em corridas das construtoras por temporada`
)}

async function _data(d3,FileAttachment){return(
d3.csvParse(await FileAttachment("constructorRaceWins@2.csv").text(), d3.autoType)
)}
...
d3.group(data, d => d.constructorName)
)}
...
new Set(data.map(d => d.constructorName))
)}

function _datevalues(d3,data){return(
Array.from(d3.rollup(data, ([d]) => d.wins_cum, d => +d.year, d => d.constructorName))
  .sort(([a], [b]) => d3.ascending(a, b))
)}

function _rank(names,d3,n){return(
function rank(value) {
  const data = Array.from(names, constructorName => ({constructorName, value: value(constructorName)}));
...
rank(constructorName => datevalues[1][1].get(constructorName))
)}
...
function _keyframes(d3,datevalues,k,rank)
{
  ...
  for ([[ka, a], [kb, b]] of d3.pairs(datevalues)) {
    for (let i = 0; i < k; ++i) {
      ...
      keyframes.push([
        ka * (1 - t) + kb * t,
        rank(constructorName => (a.get(constructorName) || 0) * (1 - t) + (b.get(constructorName) || 0) * t)
      ]);
    }
  }
  keyframes.push([kb, rank(constructorName => b.get(constructorName) || 0)]);
  return keyframes;
}


function _nameframes(d3,keyframes){return(
d3.groups(keyframes.flatMap(([, data]) => data), d => d.constructorName)
)}
...
function bars(svg) {
  ...
  return ([year, data], transition) => {
    bar = bar
      .data(data.slice(0, n), d => d.constructorName)
      .join(
...
function _color(d3)
{
  ...
  return d => scale(d.constructorName);
}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["constructorRaceWins@2.csv", {url: new URL("./files/28dfaa5b3f6d0aec334fabb917577d2612928cceca09adcca07d2a8caf3beece82e456648fc71e5e783cd58d9a3d41cfd589667915b71381338bf569bcdb5ce4.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
